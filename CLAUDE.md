...
- **Seeded:** only the two starter events in `db/seed.sql`. Events are managed by Build Club staff from `/admin` (create/edit/delete/operate); staff admins are the event organizers for every event. Staff = a row in `public.admins`, a deliberately separate table (not a self-grantable `profiles` flag); bootstrap the first admin from the SQL editor (`insert into public.admins (user_id) values ('<uuid>')`). `event_members` records participation only and has no role or authorization meaning. Admin-only writes are enforced by RLS `is_admin()`; the admin routes + nav item are gated by `useIsAdmin`.
